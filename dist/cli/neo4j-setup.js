...
const defaultConnectionManagerFactory = (config) => new Neo4jConnectionManager(config);
const defaultSchemaManagerFactory = (connectionManager, debug) => new Neo4jSchemaManager(connectionManager, undefined, debug);
/**
 ...
 */
export async function testConnection(config, debug = true, connectionManagerFactory = defaultConnectionManagerFactory) {
 ...
 */
export async function initializeSchema(config, debug = true, recreate = false, connectionManagerFactory = defaultConnectionManagerFactory, schemaManagerFactory = defaultSchemaManagerFactory) {
    const connectionManager = connectionManagerFactory(config);
    ...
    try {
        ...
        await schemaManager.createVectorIndex(config.vectorIndexName, 'Entity', 'embedding', config.vectorDimensions, config.similarityFunction, recreate);
...
//# sourceMappingURL=neo4j-setup.js.map