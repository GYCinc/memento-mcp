...
export class Neo4jSchemaManager {
     ...
     */
    constructor(connectionManager, config, debug = true) {
     ...
     */
    async dropConstraintIfExists(name) {
     ...
     */
    async createEntityConstraints(recreate = false) {
     ...
     */
    async createVectorIndex(indexName, nodeLabel, propertyName, dimensions, similarityFunction, recreate = false) {
     ...
     */
    async vectorIndexExists(indexName) {
        this.log(`Checking if vector index ${indexName} exists and is ONLINE...`);
        try {
            const result = await this.connectionManager.executeQuery('SHOW VECTOR INDEXES WHERE name = $indexName', { indexName });
            ...
            try {
                const fallbackResult = await this.connectionManager.executeQuery('SHOW INDEXES WHERE type = "VECTOR" AND name = $indexName', { indexName });
     ...
     */
    async initializeSchema(recreate = false) {
...
//# sourceMappingURL=Neo4jSchemaManager.js.map