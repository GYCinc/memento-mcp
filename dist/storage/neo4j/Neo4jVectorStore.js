...
export class Neo4jVectorStore {
    ...
    async initialize() {
        try {
            ...
            if (typeof this.schemaManager.vectorIndexExists === 'function') {
                ...
                logger.warn('vectorIndexExists method not available on schemaManager - this may be a test environment');
            }
            // Create vector index if it doesn't exist
            if (!indexExists) {
                logger.info(`Creating Neo4j vector index: ${this.indexName}`);
                if (typeof this.schemaManager.createVectorIndex === 'function') {
                    await this.schemaManager.createVectorIndex(this.indexName, this.entityNodeLabel, 'embedding', this.dimensions, this.similarityFunction);
                }
                else {
                    logger.warn('createVectorIndex method not available on schemaManager - this may be a test environment');
                }
            }
            else {
    ...
    async search(queryVector, options = {}) {
        try {
            ...
            if (queryVector.length !== this.dimensions) {
                throw new Error(`Invalid vector dimensions: expected ${this.dimensions}, got ${queryVector.length}`);
            }
            // Debug vector stats
            const vectorStats = this.calculateVectorStats(queryVector);
            logger.debug(`Neo4jVectorStore: Vector stats: min=${vectorStats.min}, max=${vectorStats.max}, avg=${vectorStats.avg}, l2norm=${vectorStats.l2Norm}`);
            ...
            logger.debug(`Neo4jVectorStore: Using vector search with limit=${limit}, minSimilarity=${minSimilarity}`);
            // Start session
            const session = await this.connectionManager.getSession();
            try {
        ...
        `, {
                ...
                logger.error(`Neo4jVectorStore: Vector search failed: ${error instanceof Error ? error.message : String(error)}`);
                return this.searchByPatternFallback(limit);
            }
            finally {
                await session.close();
            ...
            logger.error(`Neo4jVectorStore: Search failed: ${error instanceof Error ? error.message : String(error)}`);
            return this.searchByPatternFallback(options.limit ?? 5);
        }
    }
    /**
     ...
     */
    async searchByPatternFallback(limit) {
      ...
      `;
            ...
            logger.debug(`Neo4jVectorStore: Fallback search returned ${fallbackResult.records.length} results`);
     ...
     */
    async diagnosticGetEntityEmbeddings() {
        try {
            const session = await this.connectionManager.getSession();
            try {
        ...
        `;
                ...
                const indexInfo = indexResult.records.length > 0
                    ? {
        ...
        `;
                ...
                try {
                    ...
                    const testVector = Array.from({ length: this.dimensions }, () => Math.random() * 0.1 + 0.01);
                    const directQuery = `
          ...
          `;
                    ...
                    directVectorQueryResult = {
                        ...
                        sampleResult: testResult.records.length > 0
                            ? {
